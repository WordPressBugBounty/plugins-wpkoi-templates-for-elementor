			...
			const material = new THREE.ShaderMaterial({
				...
				fragmentShader: `
					...
					if(u_rgbGlitch > 0.5){
						...
						float t = u_time * u_vertexSpeed * 0.6;
						float burst = step(0.85, fract(t * 0.7));
						float intensity = u_intensity * burst * 0.7;
		...
		function setupProjectionEngine(texture, imageAspect) {
			...
			const vertexShader = `
				...
				void main() {
					...
					float ripple = sin(dist * 20.0 - t * 2.0);
					ripple *= influence;
					displacement += ripple * 0.2;
				...
				void main() {
					...
					if(u_mouseIntensity > 0.001) {
						...
						if(inside) {
							...
							float radius = u_mouseRadius * 0.4;
							float dist = distance(uv, u_mouse);
							float innerRadius = radius * 0.25;
							float fadeWidth = radius * 0.5;
							float fadeStart = innerRadius;
							float fadeEnd   = innerRadius + fadeWidth;
					...
					float noise = sin(uv.x * 20.0 + t * 2.0) * 0.005;
					noise += sin(uv.y * 18.0 - t * 1.7) * 0.005;
					...
					if(u_rgbGlitch > 0.5) {
						...
						float burst = step(0.82, fract(t * 0.7));

						float intensity = u_intensity * burst * 0.75;
