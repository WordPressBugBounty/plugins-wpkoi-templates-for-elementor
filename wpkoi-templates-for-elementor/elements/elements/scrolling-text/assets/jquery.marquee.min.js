// https://github.com/aamirafridi/jQuery.Marquee/tree/master
...
(function($){
function wpkoiVerticalCharsInit(scope) {
    $(scope).find('.wpkoi-marquee.wpkoi-vertical-chars').each(function(){

        var $marquee = $(this);

        if ($marquee.data('wpkoi-chars-init')) return;
        $marquee.data('wpkoi-chars-init', true);

        var $content = $marquee.find('.wpkoi-scrolling-content');

        if (!$content.length) return;
        var $target = $content.find('a').length ? $content.find('a') : $content;
        ...
        for (var i = 0; i < text.length; i++) {
            ...
            if (char === ' ') {
                newHTML += '<span class="char char-space">&nbsp;</span>';
            } else {
...
function wpkoiInitMarquee(scope) {
    $(scope).find('.wpkoi-marquee').each(function(){
        ...
        setTimeout(function(){
            $el.addClass('wpkoi-ready');
        }, 50);
    });
}

function wpkoiApplyVerticalHeight(scope) {
    $(scope).find('.wpkoi-marquee-vertical').each(function(){

        var $el = $(this);
        var vHeight = $el.data('vheight');

        if (vHeight) {
            $el.css('height', vHeight);
        }
    });
}

$(window).on('elementor/frontend/init', function () {
    elementorFrontend.hooks.addAction('frontend/element_ready/wpkoi-scrolling-text.default', function(scope){
        wpkoiVerticalCharsInit(scope);
		wpkoiApplyVerticalHeight(scope);
        wpkoiInitMarquee(scope);
    });
	
	elementorFrontend.hooks.addAction('frontend/element_ready/wpkoi-scrolling-images.default', function(scope){
        wpkoiVerticalCharsInit(scope);
		wpkoiApplyVerticalHeight(scope);
        wpkoiInitMarquee(scope);
    });
